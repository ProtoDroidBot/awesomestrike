	...
	local emitter = ParticleEmitter(pos)
		for i=1, math.random(-4, 4) do
			...
			if scale == 4 then
				...
				particle = emitter:Add("noxctf/sprite_bloodspray1", pos)
				--particle = emitter:Add("effects/blood_core", pos)
			end
...
function EFFECT:Init(data)
	...
	local emitter = ParticleEmitter(Pos)
		for i=1, data:GetMagnitude() do
			...
			particle:SetVelocity(VectorRand():Normalize() * math.Rand(force * 0.4, force * 0.7) + Vector(0,0,force * 0.5) + force * math.Rand(0.8, 1.2) * dir)
