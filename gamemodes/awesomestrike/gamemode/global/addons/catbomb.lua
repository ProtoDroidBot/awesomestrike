...
local function DoCatBomb(pl)
	gmod.BroadcastLua([[surface.PlaySound("vo/Citadel/gman_exit02.wav")]])
	...
	if ent:IsValid() then
		...
		timer.Simple(8.5, CatBombExplode, ent)
		CATBOMB_ENTITY = ent
	end
end
...
function CatBombExplode(prop)
	gmod.BroadcastLua([[surface.PlaySound("ambient/explosions/citadel_end_explosion1.wav")]])
	for i=1, 30 do
		timer.Simple(i*0.35, gmod.BroadcastLua, [[surface.PlaySound("ambient/explosions/explode_"..math.random(1,8)..".wav")]])
	end
